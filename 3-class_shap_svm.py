...
gamble_dataset = pd.read_csv(f"./dataset/week_gamble/220425.csv")
...
try:
    ...
    model = pickle.load(open('3-class_svm.pt','rb'))
...
try:
    # load model if possible
    model_shap = pickle.load(open('3-class-shap_svm.pt','rb'))

except:
    model_shap = make_pipeline(StandardScaler(), SVC(gamma='auto'))
    start_time = time.time()
    model_shap.fit(x_train_shap, y_train)

    print("time :", time.time() - start_time)

    # pickle.dump(model_shap, open('3-class_svm.pt','wb'))
